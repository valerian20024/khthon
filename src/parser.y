  ...
  : expression EQUAL expression
    {
      $$ = make_shared<BinOpExpr>(
        @$, BinaryOperation::Equal(), std::move($1), std::move($3)
      );
    }
  | expression LOWER expression
    {
      $$ = make_shared<BinOpExpr>(
        @$, BinaryOperation::Lower(), std::move($1), std::move($3)
      );
    }
  | expression LOWER_EQUAL expression
    {
      $$ = make_shared<BinOpExpr>(
        @$, BinaryOperation::LowerEqual(), std::move($1), std::move($3)
      );
    }
  | expression PLUS expression
    {
      $$ = make_shared<BinOpExpr>(
        @$, BinaryOperation::Plus(), std::move($1), std::move($3)
      );
    }
  | expression MINUS expression
    {
      $$ = make_shared<BinOpExpr>(
        @$, BinaryOperation::Minus(), std::move($1), std::move($3)
      );
    }
  | expression TIMES expression
    {
      $$ = make_shared<BinOpExpr>(
        @$, BinaryOperation::Times(), std::move($1), std::move($3)
      );
    }
  | expression DIVIDE expression
    {
      $$ = make_shared<BinOpExpr>(
        @$, BinaryOperation::Divide(), std::move($1), std::move($3)
      );
    }
  | expression POWER expression
    {
      $$ = make_shared<BinOpExpr>(
        @$, BinaryOperation::Power(), std::move($1), std::move($3)
      );
    }
  | expression AND expression
    {
      $$ = make_shared<BinOpExpr>(
        @$, BinaryOperation::And(), std::move($1), std::move($3)
      );
    }
  ;
